...
unsigned int buttondown[MAX_FINGERS];
...
type_b_report:
	...
	for (i = 0; i < MAX_FINGERS; i++) {
		if (check_id[i] == 1) {
			if( i == 0){
				...
				input_report_abs(ts->input_dev,		ABS_X, sisdata[i].x);
				input_report_abs(ts->input_dev,		ABS_Y, sisdata[i].y);
				input_report_abs(ts->input_dev,		ABS_PRESSURE, sisdata[i].pressure);
			}
			input_mt_slot(ts->input_dev, i);
			input_report_abs(ts->input_dev,
				ABS_MT_TRACKING_ID, sisdata[i].id);

			input_report_abs(ts->input_dev,			ABS_MT_PRESSURE, sisdata[i].pressure);
			input_report_abs(ts->input_dev,			ABS_MT_POSITION_X, sisdata[i].x);
			input_report_abs(ts->input_dev,			ABS_MT_POSITION_Y, sisdata[i].y);
	...
	for (i = 0; i < MAX_FINGERS; i++)
	{
		if( (sisdata[i].id!=-1)|| (check_id[i]!=0))
		{
#if 0		  
		  printk("SisTouch i:%2d id:%d (%d,%d) X:%4d Y=%3d P:%2d\n",
			 i, sisdata[i].id, pre_check_id[i], check_id[i], sisdata[i].x, sisdata[i].y,  sisdata[i].pressure);
		  printk("Slot:%2d Finger:%s %4dx %3dy\n",   i, ((check_id[i])?"down":" up "),  sisdata[i].x,sisdata[i].y);
#endif		  
		  input_mt_slot(ts->input_dev, i);
		  input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER,     check_id[i]);
		  input_report_abs(ts->input_dev,	    ABS_MT_POSITION_X,  sisdata[i].x);
		  input_report_abs(ts->input_dev,	    ABS_MT_POSITION_Y,  sisdata[i].y);
		  if(check_id[i]==0)
		    sisdata[i].id=-1;
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	__set_bit(ABS_MT_POSITION_X,ts->input_dev->absbit);  /* Say we are a modern Multi-Touch pad               */
	__set_bit(ABS_MT_POSITION_Y,ts->input_dev->absbit);  /* framework/native/services/inputflinger/EventHub.c */

	__set_bit(EV_SYN,   ts->input_dev->evbit);
	__set_bit(EV_KEY,   ts->input_dev->evbit);
	__set_bit(EV_ABS,   ts->input_dev->evbit);
	  set_bit(EV_ABS,   ts->input_dev->evbit); // Same ???	
	__set_bit(EV_REP,   ts->input_dev->evbit);
	__set_bit(BTN_TOUCH,ts->input_dev->keybit);
	// __set_bit(KEY_POWER,ts->input_dev->keybit);

	input_set_abs_params(ts->input_dev, ABS_X,             0, SIS_MAX_X, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_Y,             0, SIS_MAX_Y, 0, 0);
	...
	err = input_mt_init_slots(ts->input_dev, MAX_SLOTS, 0);
	if (err) {
