/*
 * linux/drivers/power/tr8_mcu_battery_subpark.c
 *
 * Battery measurement code for TR8 MCU
 *
 * based on wm97xx_battery_subpark.c
 *
 * Copyright (C) 2019 Scheidt & Bachmann GmbH
 *
...
#define  FAKE_ADC_SAMPLE        0	// TODO Change from WM97xx to TR8 MCU !!!!!!!!!!!!!!!!!!!!!!!!
...
struct tr8_mcu_pdata {
	struct tr8_mcu_batt_pdata	*batt_pdata;	/* battery data */
  	char                  		*tr8_batt_drv;  /* copied after KUKTR8BAT_get_platform_battery_data by trizeps7_probe (soc/imx/trizeps-wm9715.c) */
};

static struct tr8_mcu_pdata *g_tr8data = NULL;
...
static unsigned long tr8_mcu_read_bat(struct power_supply *bat_ps)
{
	struct tr8_mcu_pdata *tr8data = g_tr8data;
	struct tr8_mcu_batt_pdata *pdata = tr8data->batt_pdata;
#if FAKE_ADC_SAMPLE
...
//------------------------------------------------------------------------
static unsigned long tr8_mcu_read_temp(struct power_supply *bat_ps)
{
	struct tr8_mcu_pdata *tr8data = g_tr8data;
	struct tr8_mcu_batt_pdata *pdata = tr8data->batt_pdata;
...
#else
	if (pdata->temp_aux >= 0)													// @+17.07.2015 S&B HH/HL
	  return tr8_read_adc(pdata->temp_aux)*pdata->temp_mult/pdata->temp_div;
	else																		// @+17.07.2015 S&B HH/HL
		return(0);																// @+17.07.2015 S&B HH/HL
#endif
}


//------------------------------------------------------------------------
static int tr8_mcu_bat_get_property(struct power_supply *bat_ps,
...
//		printk (KERN_ERR "##### SUBTR8BATXX - CAPACITY_LEVEL:%d \n", val->intval);
		break;
	case POWER_SUPPLY_PROP_CAPACITY:
		val->intval = percent < 0 ? 0 : (percent > 100 ? 100 : percent);
//	        printk (KERN_ERR "##### SUBTR8BATXX - CAPACITY:%d \n", val->intval);
		break;	
	case POWER_SUPPLY_PROP_STATUS:
		val->intval = bat_status;
//		printk (KERN_ERR "##### SUBTR8BATXX - STATUS:%d \n", val->intval);
		break;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		if (pdata->batt_aux >= 0)
			...
			return -EINVAL;
//			 printk (KERN_ERR "##### SUBTR8BATXX - VOLTAGE_NOW:%d \n", val->intval);
		break;
	case POWER_SUPPLY_PROP_TEMP:
		if (pdata->temp_aux >= 0)
//			val->intval = tr8_mcu_read_temp(bat_ps);		// 20.11.2014 S&B HH/HL
...
static bool isCharging(struct power_supply *bat_ps)
{
	...
	val = tr8_mcu_read_temp(bat_ps);
	//printk (KERN_ERR "##### WM97XX ------------------- Charging:%lu \n", val);
	return (val < 100);
}
#endif

//------------------------------------------------------------------------
static u32 get_capability_percent(struct tr8_mcu_batt_pdata *pdata)
{
	u8 i;
...
static u32 get_voltage(struct power_supply *bat_ps)
{
	...
	for (i = 0; i < ADC_SAMPLE_COUNT; i++) 
	{
		if (charger_online == 0) 
		{
			...
			volt[i] = tr8_mcu_read_bat(bat_ps)-offset_discharger;
		} 
		else 
		{
			volt[i] = tr8_mcu_read_bat(bat_ps)-offset_charger;
		}
	}
	sort(volt, i, 4, cmp_func, NULL);
	//	for (i = 0; i < ADC_SAMPLE_COUNT; i++)
	//	printk (KERN_ERR "volt_sorted[%2d] : %d\n", i, volt[i]);
...
static u32 get_voltage_temp(struct power_supply *bat_ps)
{
	...
	for (i = 0; i < ADC_SAMPLE_COUNT; i++) 
	{
		volt[i] = tr8_mcu_read_temp(bat_ps);
	}
	sort(volt, i, 4, cmp_func, NULL);
	voltage_data = (volt[3] + volt[ADC_SAMPLE_COUNT - 2]) / 2;
	return voltage_data;
...
//------------------------------------------------------------------------
static void tr8_mcu_bat_external_power_changed(struct power_supply *bat_ps)
{
//  printk (KERN_ERR "######## SUBTR8BATXX - EXTERN POWER CHANGED\n");
  schedule_work(&bat_work);
}


//------------------------------------------------------------------------
static void tr8_mcu_bat_update(struct power_supply *bat_ps)
{
	struct tr8_mcu_pdata *tr8data = g_tr8data;
	struct tr8_mcu_batt_pdata *pdata = tr8data->batt_pdata;
...
// 	printk (KERN_ERR "##### WM97XX - voltage:%d percent: %d count:%d status %d\n", voltage_uV, percent, batChangeCount, new_status);
	// ##### 130801 EG
	...
	if(voltage_uV > 100000) // 100mV !?
	  if ((old_status != bat_status) || changed_flag)
	  {
	    ...
	    if( pdata->charge_gpio >= 0 )					// 20.11.2014 S&B HH/HL
	      printk (KERN_ERR "##### SUBTR8BATXX - POWER SUPPLY CHANGED %d percent, GPIO=%d\n", 
...
static struct power_supply_desc bat_psy_desc = {
	...
	.get_property		= tr8_mcu_bat_get_property,
	.external_power_changed = tr8_mcu_bat_external_power_changed,
	.use_for_apm		= 1,
};


//------------------------------------------------------------------------
static void tr8_mcu_bat_work(struct work_struct *work)
{
    if( tr8_adc_get_i2c_client() != NULL )
    {
		tr8_mcu_bat_update(bat_psy);
    }
}

//------------------------------------------------------------------------
static void tr8_mcu_bat_delayed_work(struct work_struct *work)
{
	schedule_work(&bat_work);	
...
//------------------------------------------------------------------------
static irqreturn_t tr8_mcu_chrg_irq(int irq, void *data)
{
//  	printk (KERN_ERR "########## SUBTR8BATXX - IRQ - GpioBatChange\n");
	batChangeCount = 0;
	schedule_work(&bat_work);
...
//------------------------------------------------------------------------
#ifdef CONFIG_PM
static int tr8_mcu_bat_suspend(struct device *dev)
{
...
//------------------------------------------------------------------------
static int tr8_mcu_bat_resume(struct device *dev)
{
	schedule_delayed_work(&bat_delayed_work, DELAY * HZ);
//	AndroidBatChangeNotify();
...
//------------------------------------------------------------------------
//------------------------------------------------------------------------
static const struct dev_pm_ops tr8_mcu_bat_pm_ops = {
	.suspend	= tr8_mcu_bat_suspend,
	.resume		= tr8_mcu_bat_resume,
};
#endif

//------------------------------------------------------------------------
#if defined(CONFIG_OF)
static const struct of_device_id tr8_mcu_bat_dt_ids[] = {
	{ .compatible = "sub,tr8_mcu_cap"},
	{ .compatible = "sub,tr8_cap" 	 },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, tr8_mcu_bat_dt_ids);
...
static struct tr8_mcu_pdata *tr8_mcu_bat_of_populate_pdata(
		struct device *dev)
{
	struct device_node 		 *of_node 	= dev->of_node;
	struct tr8_mcu_pdata 	 *tr8data 	= dev->platform_data;
	struct tr8_mcu_batt_pdata *pxBattData= NULL;

	if (!of_node || tr8data)
		return tr8data;

	tr8data = devm_kzalloc(dev, sizeof(struct tr8_mcu_pdata),
				GFP_KERNEL);
	if (!tr8data)
		return tr8data;

	pxBattData= devm_kzalloc(dev, sizeof(struct tr8_mcu_batt_pdata),
				GFP_KERNEL);
	if (!pxBattData)
		return NULL;
	...
	if (of_property_read_u32(of_node, "batt_aux", &pxBattData->batt_aux)){
		...
		printk (KERN_ERR  "tr8_mcu_bat_of_populate_pdata: batt_aux = 0x%x \n", pxBattData->batt_aux);
	...
	if (of_property_read_u32(of_node, "temp_aux", &pxBattData->temp_aux)){
		...
		printk (KERN_ERR  "tr8_mcu_bat_of_populate_pdata: temp_aux = 0x%x \n", pxBattData->temp_aux);
	...
	if (of_property_read_u32(of_node, "batt_mult", &pxBattData->batt_mult)){
		...
		printk (KERN_ERR  "tr8_mcu_bat_of_populate_pdata: batt_mult = %d \n", pxBattData->batt_mult);
	...
	if (of_property_read_u32(of_node, "batt_div", &pxBattData->batt_div)){
		...
		printk (KERN_ERR  "tr8_mcu_bat_of_populate_pdata: batt_div = %d \n", pxBattData->batt_div);
	...
	if (of_property_read_u32(of_node, "temp_mult", &pxBattData->temp_mult)){
		...
		printk (KERN_ERR  "tr8_mcu_bat_of_populate_pdata: temp_mult = %d \n", pxBattData->temp_mult);
	...
	if (of_property_read_u32(of_node, "temp_div", &pxBattData->temp_div)){
		...
		printk (KERN_ERR  "tr8_mcu_bat_of_populate_pdata: temp_div = %d \n", pxBattData->temp_div);
	...
	if (of_property_read_u32(of_node, "min_voltage", &pxBattData->min_voltage)){
		...
		printk (KERN_ERR  "tr8_mcu_bat_of_populate_pdata: min_voltage = %d \n", pxBattData->min_voltage);
	...
	if (of_property_read_u32(of_node, "max_voltage", &pxBattData->max_voltage)){
		...
		printk (KERN_ERR  "tr8_mcu_bat_of_populate_pdata: max_voltage = %d \n", pxBattData->max_voltage);
	...
	if (of_property_read_u32(of_node, "batt_tech", &pxBattData->batt_tech)){
		...
		printk (KERN_ERR  "tr8_mcu_bat_of_populate_pdata: batt_tech = %d \n", pxBattData->batt_tech);
	...
	printk (KERN_ERR  "tr8_mcu_bat_of_populate_pdata: batt_name = %s \n", pxBattData->batt_name);

	return tr8data;
}
#endif

static int tr8_mcu_bat_probe(struct platform_device *dev)
{
	int ret = 0;
	int props = 2;	/* POWER_SUPPLY_PROP_PRESENT u. POWER_SUPPLY_PROP_STATUS*/
	int i = 0;
//	struct tr8_mcu_pdata *tr8data = dev->dev.platform_data;
	struct tr8_mcu_pdata *tr8data = g_tr8data;
	struct tr8_mcu_batt_pdata *pdata;
	struct power_supply_config psy_cfg = {};


	printk (KERN_ERR  "tr8_mcu_bat_probe: START!\n");

	if (!tr8data) {
		tr8data = tr8_mcu_bat_of_populate_pdata(&dev->dev);
		if(!tr8data){
		...
		if(dev->dev.of_node)	// of settings given?
			g_tr8data = tr8data;
		printk (KERN_ERR  "tr8_mcu_bat_probe: 1st END after tr8_mcu_bat_of_populate_pdata()!!!!!!!!\n");
// TODO: With WM97xx we need an 2nd call		return 0;
	}
...
//	platform_set_drvdata(dev, tr8data);  // Set it back to device struct
	...
	if (!pdata) {
		dev_err(&dev->dev, "tr8_mcu_bat_probe: No platform_data supplied\n");
		return -EINVAL;
	}
...
//	if (gpio_is_valid(pdata->charge_gpio)) {
	if ((pdata->charge_gpio) > 0 && gpio_is_valid(pdata->charge_gpio)) {
		...
		ret = request_irq(gpio_to_irq(pdata->charge_gpio),
				tr8_mcu_chrg_irq, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
				"Battery change", dev);
		if (ret)
			goto err2;
...
// + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + 20.11.2014 S&B HH/HL
	...
	INIT_WORK(&bat_work, tr8_mcu_bat_work);
	INIT_DELAYED_WORK(&bat_delayed_work, tr8_mcu_bat_delayed_work);
...
//	bat_psy = power_supply_register(&dev->dev, &bat_psy_desc, &psy_cfg);
	...
	{	    
	  ...
	  // schedule_delayed_work(&bat_delayed_work, 60 * HZ);
		...
		ret = PTR_ERR(bat_psy);
		printk (KERN_ERR  "tr8_mcu_bat_probe: Error %d by power_supply_register() call!\n", ret);
		goto err4;
	}
//	AndroidBatChangeNotify();

	printk (KERN_ERR  "tr8_mcu_bat_probe: 2nd END!\n");
	return 0;
err4:
	printk (KERN_ERR  "tr8_mcu_bat_probe: err4 exit!\n");
	kfree(prop);
err3:
	printk (KERN_ERR  "tr8_mcu_bat_probe: err3 exit!\n");
	if (gpio_is_valid(pdata->charge_gpio))
		free_irq(gpio_to_irq(pdata->charge_gpio), dev);
err2:
	printk (KERN_ERR  "tr8_mcu_bat_probe: err2 exit!\n");
	if (gpio_is_valid(pdata->charge_gpio))
		gpio_free(pdata->charge_gpio);
err:
	printk (KERN_ERR  "tr8_mcu_bat_probe: END with error!\n");
	return ret;
}

static int tr8_mcu_bat_remove(struct platform_device *dev)
{
	struct tr8_mcu_pdata *tr8data = dev->dev.platform_data;
	struct tr8_mcu_batt_pdata *pdata = tr8data->batt_pdata;
...
//------------------------------------------------------------------------
//------------------------------------------------------------------------
static struct platform_driver tr8_mcu_bat_driver = {
	.driver		= {
//		.name			= "wm97xx-battery-subp",
		.name			= "tr8-mcu-battery",
		.owner			= THIS_MODULE,
		.of_match_table = tr8_mcu_bat_dt_ids,
#ifdef CONFIG_PM
		.pm		= &tr8_mcu_bat_pm_ops,
#endif
	},
	.probe		= tr8_mcu_bat_probe,
	.remove		= tr8_mcu_bat_remove,
};

module_platform_driver(tr8_mcu_bat_driver);


MODULE_LICENSE("GPL v2");
MODULE_AUTHOR("Scheidt & Bachmann GmbH");
MODULE_DESCRIPTION("TR8 MCU subp battery driver for Linux");
