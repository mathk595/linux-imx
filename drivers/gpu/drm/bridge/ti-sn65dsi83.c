...
//				     "failed to create dsi device\n");
	...
	dev_warn(ctx->dev, "Call mipi_dsi_attach with %d dsi->lanes! for Resolution [ %d x %d ]\n",
			 dsi->lanes, ctx->mode.hdisplay, ctx->mode.vdisplay);
...
static void sn65dsi83_enable(struct drm_bridge *bridge)
{
	...
	if(ctx->mode.hdisplay == 320) { // S&B special 320x240 display
	...
	dev_info(ctx->dev, "Video mode flags 0x%x, value write to Register 0x%x == 0x%x\n", ctx->mode.flags, REG_LVDS_FMT, val);
	...
	regmap_write(ctx->regmap, REG_LVDS_VCOM, val);
	...
	dev_info(ctx->dev, "Enable: Resolution [ %d x %d ]\n",
...
static void sn65dsi83_disable(struct drm_bridge *bridge)
{
	...
	dev_info(ctx->dev, "Disable: Resolution [ %d x %d ]\n",
			ctx->mode.hdisplay, ctx->mode.vdisplay);
...
static int sn65dsi83_parse_dt(struct sn65dsi83 *ctx, enum sn65dsi83_model model)
{
	...
	ctx->burst_mode = of_property_read_bool(endpoint, "burst-mode");
