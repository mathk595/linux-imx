				    ...
				    struct drm_bridge_state *old_bridge_state)
{
	...
	if(mode->hdisplay == 320) { // S&B special 320x240 display
...
static int sn65dsi83_host_attach(struct sn65dsi83 *ctx)
{
	...
	if (!host) {
		dev_err(dev, "failed to find dsi host: %p\n", ctx->host_node);
		return -EPROBE_DEFER;
	}
	...
	dev_warn(ctx->dev, "Call mipi_dsi_attach with %d dsi->lanes! \n",
			 dsi->lanes);
