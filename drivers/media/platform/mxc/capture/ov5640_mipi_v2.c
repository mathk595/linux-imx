...
static inline void ov5640_power_down(struct ov5640 *sensor, int enable)
{
	...
	{
	    pr_err("%s:wrong GPIO %d\n", __func__,sensor->pwn_gpio);	  
	    return;
	}
	
	gpio_set_value(sensor->pwn_gpio, (enable) ? 1: 0);
...
static int ov5640_download_autofocus(struct ov5640 *sensor)
{
	u8  r;
	int sval = ov5640_read_reg(sensor, 0x3000, &r);
	if (0 > sval) {
	...
	r |= 0x20;
	ov5640_write_reg(sensor, 0x3000,r);
	...
	/* VCM control4 */
	ov5640_write_reg(sensor, 0x3606,0x07);
	return sval;
}

static int status_auto_focus(struct ov5640 *sensor)
{
    u8  r;
	int retval;
	ov5640_read_reg(sensor, FW_STATUS, &r);
	switch(r)
	{
		case 0x20:
			retval = V4L2_AUTO_FOCUS_STATUS_REACHED;
			break;	
		case 0x00:
		case 0x10:
			retval = V4L2_AUTO_FOCUS_STATUS_BUSY;
			break;
		case 0x70:
		default:
			retval = V4L2_AUTO_FOCUS_STATUS_FAILED;
		break;
	}

	//printk("status_auto_focus 0x%x -> 0x%x\n",r , retval);
	return retval;
}


static int trigger_auto_focus(struct ov5640 *sensor, int mode)
{
    ov5640_write_reg(sensor, CMD_ACK, 0x01);
	ov5640_write_reg(sensor, CMD_MAIN, mode);
	return 0;
}
			    ...
			    enum ov5640_mode mode, enum ov5640_mode orig_mode)
{
	...
	if (mode == ov5640_mode_INIT) {
		...
		retval = ov5640_download_autofocus(sensor);
		if (retval < 0) {
		...
		struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	if (fie->width == 0 || fie->height == 0 ||
	    fie->code == 0) {
	        dev_warn(dev, "Please assign pixel format, width and height. width:%d height:%d code:%d\n",
			  fie->width,fie->height,fie->code);
		return -EINVAL;
	}
...
static int ov5640_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch (ctrl->id) {
		case V4L2_CID_AUTO_FOCUS_STATUS:
			ctrl->value = status_auto_focus(sensor);
			break;
		default:
...
static int ov5640_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch (ctrl->id) {
		case V4L2_CID_FOCUS_AUTO:
			trigger_auto_focus(sensor, 0x04);
			break;
		case V4L2_CID_AUTO_FOCUS_STOP:
			trigger_auto_focus(sensor, 0x08);
			break;
		default:
			pr_debug("default case\n");
			retval = -EINVAL;
			...
			const struct i2c_device_id *id)
{
	...
	dev_info(dev, "ov5640_mipi_v2.c ov5640_probe(.....)\n");	
	...
	if (retval < 0 || chip_id_low != 0x40) {
		dev_warn(dev, "Camera is not found chip_id_low:0x%x\n", (unsigned int) chip_id_low );
		ov5640_regualtor_disable();
		clk_disable_unprepare(sensor->sensor_clk);
		return -ENODEV;
	}
	...
	OV5640_stream_off(sensor);
	dev_info(dev, "ov5640_mipi_v2.c ov5640_probe: Camera is found\n");
	return retval;
}
